...
export default function Benefits() {
  return (
    <section className="bg-[#f7f1eb] px-6 py-16 md:py-24">
          ...
          {benefits.map((item) => {
            ...
            return (
              <motion.article
                key={item.title}
                variants={fadeUp}
                className="group rounded-[24px] bg-white p-7 shadow-sm transition-all hover:shadow-xl border border-transparent hover:border-[#f2a9c0]"
              >
                <div className="mb-5 inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-[#fce7ee] text-[#d97c97] transition-transform group-hover:rotate-12">
                  <Icon size={28} />
                </div>
                <h3 className="mb-3 text-xl font-bold text-[#1d4448]">{item.title}</h3>
                <p className="leading-relaxed text-[#4d5b59] text-sm">{item.description}</p>
              </motion.article>
            );
          })}
        </motion.div>
